...
The full programming codes used in this study for the Python and R languages, and GMT scripts are publicly provided in the open GitHub repository of the authors using the following link: \href{https://github.com/paulinelemenkova/Image_Processing_Mali_IND_Scripts}{https://github.com/paulinelemenkova/Image\_Processing\_Mali\_IND\_Scripts}. In the subsections below we describe the scripts and technical approaches in details, with the comments on the most important snippets on the codes.
...
\centering
	...
	\vspace*{20pt}\caption{The Landsat-8 OLI/TIRS satellite image downloaded from the EarthExplorer repository, USGS. Mali, Mopti region of the Inner Niger Delta. Background image: ESRI World imagery.
	\label{fig02}}
\end{figure}
...
\centering
	\includegraphics[width=14.5 cm]{Fig_04.jpg}
		\vspace*{20pt}\caption{Processing Landsat OLI/TIRS satellite image in RStudio for extracting NDVI values.
	\label{fig04}}
\end{figure}
...
# Computing Normalized Difference Vegetation Index 
# (NDVI) = (NIR - R) / (NIR + R). 
# For Landsat 8-9, NIR = 5, red = 4: (Band 5 - Band 4) / (Band 5 + Band 4).
library(terra)
...
\newpar The calculation of the correct Soil Adjusted Vegetation Index (SAVI) corrects the NDVI for the effects of soil brightness in desert regions, bare soil and other places where vegetative cover is low. The general formula used for calculation SAVI is as follows SAVI = ((Band 5 – Band 4) / (Band 5 + Band 4 + 0.5)) * (1.5). Note that the numeration of these bands corresponds to the 4 and 3, because the numeration in bands of RasterObject starts from 0 (zero). Therefore, in formula below, the NIR and Red bands are assigned numbers of 4 and 3 in the 'lapp function', as showed below. Using R syntax where NIR = Band 5, and red channel correspond to the Band 4, the computing is performed as follows in the code snippet: 
...
plt.title('Pearson correlation for land cover types in Inner Niger Delta, 
	   Mali', fontsize=13, fontfamily='serif')
...
\subsection*{The results of clustering for Landsat OLI/TIRS images on Inner Niger Delta, Mali on 2013 (10 November 2013)}
\begin{scriptsize}
\begin{verbatim}
...
\subsection*{The results of clustering for Landsat OLI/TIRS images on Inner Niger Delta, Mali on 2015 (18 November 2015)}
...
\subsection*{The results of clustering for Landsat OLI/TIRS images on Inner Niger Delta, Mali on 2018 (24 November 2018)}
...
\subsection*{The results of clustering for Landsat OLI/TIRS images on Inner Niger Delta, Mali on 2020 (29 November 2020)}
...
\subsection*{The results of clustering for Landsat OLI/TIRS images on Inner Niger Delta, Mali on 2021 (25 November 2021)}
\begin{scriptsize}
\begin{verbatim}
unsuperClass results
...
\subsection*{The results of clustering for Landsat OLI/TIRS images on Inner Niger Delta, Mali on 2022 (13 November 2022)}
\begin{scriptsize}
\begin{verbatim}
unsuperClass results
...
{\bf Acknowledgements.} The authors would like to thank the editors and the reviewers for review of the manuscript. This project was supported by the Federal Public Planning Service Science Policy or Belgian Science Policy Office, Federal Science Policy -- BELSPO (B2/202/P2/SEISMOSTORM). 
