...
\newpar The process of computing SAVI vegetation index is performed in R library 'terra' using the code in Appendix in Listing \ref{lst:03} (here: example for 2021, repeated likewise for all other images).
...
\newpar Computing the Enhanced Vegetation Index (EVI) was based using the equation of Landsat 4 and 5 bands: EVI2 = 2.4 * (Band 5 - Band 4) / (Band 5 + Band 4 + 1) \citep{HUETE2002195}. In R language, this formula was defined and converted using its syntax as follows:  
...
\newpar The calculation and visualization of the Enhanced Vegetation Index (EVI) was based using the script in Appendix in Listing \ref{lst:04}.
...
\newpar The correlation analysis was performed using plots of the non-parametric Kendall correlation method \citep{Kendall1938} and Pearson correlation method \citep{Pearson1895}. Both methods are used to measure the association between the two measured quantities, that is, land cover classes and the probability of the correct association of the pixels assigned to these classes. In Python, these algorithms were implemented by the code presented in Listing \ref{lst:06} in the Appendix (here a case for Pearson method, modified and applied for the case of Kendall method). The correlation between two computed land cover classes is high when observations have a similar values, and vice versa.
...
\newpar The results of the NDVI computation show changes in vegetation from 2013 to 2022, Figure \ref{fig06}.
...
\end{tabularx}
 \begin{tablenotes}
      \scriptsize
      	\item \emph{Note for Table 2} 'nrow' -- number of rows on the image matrix, 'ncol' -- number of rows, 'nlyr' -- number of layers, $x_{min}$ and $y_{min}$ -- the minimal values of pixel on the image in x and y dimensions of the pixel's matrix, $x_{max}$ and $y_{max}$ -- the maximal values of pixel on the image in x and y dimensions of the pixel's matrix, 'min' -- minimal value of the VI, 'max' -- maximal value of the VI.
    \end{tablenotes}
  \end{threeparttable}
\end{table}

Figure \ref{fig07} shows the frequency of the NDVI values by pixels on the image. Although theoretically, the NDVI ranges from 01.0 to +1.0, in this particular area of Mali, the actual range varies from -0.3 to 0.5 for the month of November for all years.
...
\vfill \vspace{1mm}
	...
	\begin{subfigure}[b]{.35\textwidth}
		...
		\caption{2022}
	\end{subfigure}%
\vspace*{20pt}\caption{Histograms of SAVI based on the Landsat 8-9 images of Inner Niger Delta: (\textbf{a}) 2013, (\textbf{b}) 2015, (\textbf{c}) 2018, (\textbf{d}) 2020, (\textbf{e}) 2021, (\textbf{f}) 2022. Plots: RStudio. Source: authors.}\label{fig09}
\end{figure}
...
\end{tabularx}
 \begin{tablenotes}
      \scriptsize
      	\item \emph{Note for Table 4} 'nrow' -- number of rows on the image matrix, 'ncol' -- number of rows, 'nlyr' -- number of layers, $x_{min}$ and $y_{min}$ -- the minimal values of pixel on the image in x and y dimensions of the pixel's matrix, $x_{max}$ and $y_{max}$ -- the maximal values of pixel on the image in x and y dimensions of the pixel's matrix, 'min' -- minimal value of the VI, 'max' -- maximal value of the VI.
    \end{tablenotes}
  \end{threeparttable}
\end{table}
